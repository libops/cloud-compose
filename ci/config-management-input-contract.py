...
for marker in (
    ...
    '</work/ci/remote/config-management-verify.sh',
    "printf -v remote_command '%q '",
):
    if marker not in cloud_smoke_inner:
        fail(f"hosted adapter checked remote-program marker is missing: {marker!r}")
if "bash -s" in cloud_smoke_inner or "<<'REMOTE'" in cloud_smoke_inner:
    fail("hosted adapter smoke still sends an embedded Bash program over SSH")
if "bootstrap_wait_seconds: 1200" not in remote_salt_deploy:
...
for marker in (
    ...
    if marker not in remote_verification:
        fail(f"checked remote verification program is missing: {marker!r}")
if "bash -lc" in remote_verification or "bash -c" in remote_verification:
    fail("checked remote verification still embeds its healthcheck as a Bash argument")
...
for marker in (
    ...
    'type=bind,src=${container_entrypoint},dst=/usr/local/libexec/cloud-compose-config-management-smoke,readonly',
    ...
    'install -m 0700 /dev/stdin $diagnostics_path',
    'ssh_cmd "$home_dir" "$key_path" "$host" "$diagnostics_path"',
):
    if marker not in cloud_smoke_driver:
        fail(f"hosted adapter smoke bounded execution/diagnostics marker is missing: {marker!r}")
if 'exec bash ci/config-management-cloud-smoke-inner.sh' not in cloud_smoke_container:
    fail("hosted adapter smoke container does not invoke its checked entrypoint directly")
if "bash -lc" in cloud_smoke_container or "bash -c" in cloud_smoke_container:
    fail("hosted adapter smoke container still embeds its entrypoint as a Bash argument")
if 'dst=/usr/local/libexec/cloud-compose-config-management-smoke,readonly' not in local_smoke_driver:
    fail("local adapter smoke does not mount its checked container entrypoint read-only")
if 'exec bash ci/config-management-smoke-inner.sh' not in local_smoke_container:
...
for marker in (
    ...
    "journalctl -u cloud-compose-bootstrap.service",
    "journalctl -u cloud-compose.service",
    "docker ps -a",
):
    if marker not in remote_diagnostics:
        fail(f"hosted adapter remote diagnostics are missing {marker!r}")
for marker in (
    ...
    if marker not in cloud_smoke_workflow:
        fail(f"hosted adapter workflow bound is missing: {marker!r}")
